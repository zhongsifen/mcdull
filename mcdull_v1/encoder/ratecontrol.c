...
void x264_ratecontrol_init_reconfigurable( x264_t *h, int b_init )
{
    ...
    if( h->param.rc.i_vbv_max_bitrate > 0 && h->param.rc.i_vbv_buffer_size > 0 )
    {
        ...
        int bufsize = lrint(h->param.rc.i_vbv_max_bitrate / rc->fps);
...
void x264_ratecontrol_mb( x264_t *h, int bits )
{
    ...
    if( y < h->i_threadslice_end-1 )
    {
        ...
        float qp_max, buffer_left_planned, slice_size_planned, max_frame_error, size_of_other_slices, rc_tol;
        float step_size = 0.5;
        ...
        max_frame_error = X264_MAX( 0.05, 1.0 / (h->mb.i_mb_height) );
        ...
        b1 = lrint(predict_row_size_sum( h, y, rc->qpm ) + size_of_other_slices);
...
static double get_diff_limited_q(x264_t *h, ratecontrol_entry_t *rce, double q)
{
    ...
    {
        float mask = 1.f - powf( (float)rce->i_count / rcc->nmb, 2 );
...
static int update_vbv( x264_t *h, int bits )
{
    ...
    {
        filler = lrint(ceil( (rct->buffer_fill_final - buffer_size) / (8. * h->sps->vui.i_time_scale) ));
